...
import { PropsReportes } from '../../Views/pdfs/pedidos/ReportesDropiFechasPDF';
...
export const TemplateReportesFechas: React.FC<Props> = ({ datos }) => {
    ...
    const estados = [
        {
            nombre: "Anulado",
            estilo: styles.lblAnulado
        },
        {
            nombre: "Recibido",
            estilo: styles.lblRecibido
        },
        {
            nombre: "Impreso 📰",
            estilo: styles.lblImpreso
        },
        {
            nombre: "En revision",
            estilo: styles.lblRevision
        },
        {
            nombre: "Revisado  🌟",
            estilo: styles.lblRevisado
        },
        {
            nombre: "Alistado 📦",
            estilo: styles.lblAlistado
        }
    ]
        ...
        <Document>
            <Page size={'LETTER'} style={styles.page}>
                ...
                <View style={styles.subtitlesContainer}>
                    ...
                    <Text style={[styles.tableHeader, styles.lblEstado]}>Estado</Text>
                    <Text style={[styles.tableHeader, styles.lblPago]}>Pago</Text>
                </View>
                        ...
                        datos.map((item, index) => {
                            return (
                                <View style={styles.rowContainer} key={index}>
                                    ...
                                    <View style={[styles.tableBody, styles.lblEstado, styles.direccionEstado]}>
                                        {item.intEstado == -1 ? (
                                            <Text style={[estados[0].estilo]}>{estados[0].nombre}</Text>
                                        ) : (
                                            <Text style={[estados[item.intEstado] ? estados[item.intEstado].estilo : estados[2].estilo]}>{estados[item.intEstado] ? estados[item.intEstado].nombre : estados[2].nombre}</Text>
                                        )}

                                        {item.devolucion == 1 && (
                                            <Text style={[styles.devolucion]}>Devolución</Text>
                                        )}

                                    </View>
                                    <Text style={[styles.tableBody, styles.lblPago, item.pago == 1 ? styles.pago : styles.sinPagar]}>{item.pago == 1 ? "Pagado" : "Sin pagar"}</Text>
